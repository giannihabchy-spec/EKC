...
from ml.config import lgbm_tuning
from ml.forecast_daily_sales import recursive_forecast


n_ests = lgbm_tuning['n_est']
learning_rates = lgbm_tuning['learning_rate']
max_depths = lgbm_tuning['max_depth']
subsamples = lgbm_tuning['subsample']
colsample_bytrees = lgbm_tuning['colsample_bytree']
num_leaves_list = lgbm_tuning['num_leaves']


def fit_lgbm(s: pd.Series, freq: str = "D") -> dict:
    ...
    new_pred = model.predict(new_x_val)
    ...
    forecast = recursive_forecast(model, s, final_features, freq)
