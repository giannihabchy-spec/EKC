...
with col3:
    save_mode = st.selectbox("Select Save Mode", options=["Do not Save", "Save"], index=0, key="ptdb_save")
...
if st.button("▶ Run", type="primary", use_container_width=True):
    ...
    with st.status("Filtering...", expanded=True) as filter_st:
        ...
        prep_name = preprocessor.replace("_", " ")
        ...
        if save_mode == 'Save':
            save_cleaned_data(data, base_folder, f'{prep_name}.xlsx')
