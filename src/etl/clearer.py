    ...
    try:
        ...
        for sht in wb.sheets:
            ...
            if sht.api.ListObjects.Count > 0:
                ...
                headers = table.HeaderRowRange.Value[0]
                headers = [normalize_column_name(c) for c in headers]
                ...
                # Excel returns a tuple for one row sometimes. Normalize it.
                ...
                for row_idx, row in enumerate(data, start=1):
                    ...
                    if any(is_invalid(row[col_idx]) for col_idx in col_indexes):
                ...
                first_excel_row = table.DataBodyRange.Row + first_bad_row - 1
                last_excel_row = table.DataBodyRange.Row + row_count - 1

                sht.range(f"{first_excel_row}:{last_excel_row}").delete()
            ...
            # -------------------------
                ...
                for offset, row in enumerate(data):
                    ...
                    if any(is_invalid(row[col_idx]) for col_idx in col_indexes):
                        first_bad_row = first_data_row + offset
                        break
                ...
                sht.range(f"{first_bad_row}:{last_row}").delete()
